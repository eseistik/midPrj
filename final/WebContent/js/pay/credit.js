...
// if (!window.L) { window.L = function () { console.log(arguments);} } //
// optional EZ quick logging for debugging
(function($) {
	...
	var Plugin = function(target, options) {
		...
		/** #### OPTIONS #### */
		this.options = $.extend(true, // deep extend
		{
			DEBUG : false
		}, options);
	...
	Plugin.prototype._init = function(target, options) {
		...
		base.cvc.payment('formatCardCVC').data('payment-error-message',
		...
		base.number
				.on(
						'input',
						function() {
							base.cardType = $.payment.cardType(base.number
									.val());
							...
							if (base.cardType) {
								...
								// Also set an icon
								var icon = base._cardIcons[base.cardType] ? base._cardIcons[base.cardType]
		...
		base.number.on('change', function() {
			base._setValidationState($(this), !$.payment.validateCardNumber($(
					this).val()));
		});

		// Validate card expiry on change
		base.exp.on('change', function() {
			base._setValidationState($(this), !$.payment.validateCardExpiry($(
					this).payment('cardExpiryVal')));
		});

		// Validate card cvc on change
		base.cvc.on('change', function() {
			base._setValidationState($(this), !$.payment.validateCardCVC(
					$(this).val(), base.cardType));
		});
	};
	...
	Plugin.prototype.valid = function() {
		...
		var exp_valid = $.payment.validateCardExpiry(base.exp
				.payment('cardExpiryVal'));
		var cvc_valid = $.payment
				.validateCardCVC(base.cvc.val(), base.cardType);
	...
	Plugin.prototype._setValidationState = function(el, erred) {
		...
		if (erred) {
			fg.append("<span class='text-danger payment-error-message'>"
					+ el.data('payment-error-message') + "</span>");
		}
		return this;
	}

	/**
	 * EZ Logging/Warning (technically private but saving an '_' is worth it
	 * imo)
	 */
	 ...
	 * 
	 * Method options are stored via jQuery's data() method in the relevant
	 * element(s) Notice, myActionMethod mustn't start with an underscore (_) as
	 * this is used to indicate private methods on the PLUGIN class.
	 */
	$.fn[PLUGIN_NS] = function(methodOrOptions) {
		...
		if (instance && methodOrOptions.indexOf('_') != 0
			...
			return instance[methodOrOptions](Array.prototype.slice.call(
					arguments, 1));

			// CASE: argument is options object or empty = initialise
		} else if (typeof methodOrOptions === 'object' || !methodOrOptions) {

			instance = new Plugin($(this), methodOrOptions); // ok to
																// overwrite if
																// this is a
																// re-init
			...
			$.error('Plugin must be initialised before using method: '
					+ methodOrOptions);
